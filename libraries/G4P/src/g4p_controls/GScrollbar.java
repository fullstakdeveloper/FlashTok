...
class GScrollbar extends GAbstractControl {
	...
	public void setFiller(float filler) {
		this.filler = filler;
	}
	/**
	...
	protected boolean isOverThumb(float px, float py) {
		float p = (px - 16) / (width - 32); // position along the track
	...
	protected void updateBuffer() {
		if (bufferInvalid) {
			...
			float thumbWidth = Math.max((width - 32)* filler, height) ;
//			System.out.println("Width: " + width + "    Height: " + height + "            Thumb width: " + thumbWidth);
