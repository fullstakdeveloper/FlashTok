...
public abstract class GAbstractControl implements PConstants, GConstants, GConstantsInternal {
	 ...
	 * @param enable true to show tips, false to hide tips
	 */
	public void setTipEnabled(boolean enable) {
		if (tip != null) tip.setEnabled(enable);
	}
	...
	public static class Z_Order implements Comparator<GAbstractControl> {
		...
		public int compare(GAbstractControl c1, GAbstractControl c2) {
			...
			Integer y1 = new Integer((int) -c1.y), y2 = new Integer((int) -c2.y);
			Integer cmpY = y1.compareTo(y2);
			if(cmpY != 0) 
				return cmpY;
			Integer x1 = new Integer((int) -c1.x), x2 = new Integer((int) -c2.x);
